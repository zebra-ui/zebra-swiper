...
export default function loopFix({
	...
	if (isPrev) {
		prependSlidesIndexes.forEach((index) => {
			...
			// const prependData = 
			swiper.native.value.unshift(swiper.native.value[index])
			swiper.native.value.splice(swiper.native.value.length - 1, 1)
	...
	if (isNext) {
		appendSlidesIndexes.forEach((index) => {
			...
			swiper.native.value.splice(0, 1)
			swiper.native.value.push(swiper.native.value[index])
